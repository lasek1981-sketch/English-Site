...
// ====== SRS ======
const STORAGE_KEY = "english_sprint_srs_v4";
function loadSRS() {
...
function speak(text, { lang = "en-US", rate = 0.95, pitch = 1.0 } = {}) {
  if (!("speechSynthesis" in window)) {
    alert("Twoja przeglądarka nie wspiera TTS.");
    return;
  }
...
function loadThemePref() { return localStorage.getItem(THEME_KEY) || "auto"; }
...
// Test TTS
voiceTest.addEventListener("click", () => speak("This is a test of English pronunciation.", { rate: 0.95 }));

// Render list (Idioms & Phrasals) z przyciskiem audio
function renderLists() {
  idiomList.innerHTML = IDIOMS.map(x => `
    <li>
      <button class="speak btn ghost" aria-label="Odtwórz wymowę idiomu" data-text="${x.i}">🔊</button>
      <span class="en">${x.i}</span> — <span class="pl">${x.pl}</span>
      <span class="ex">(${x.ex})</span>
    </li>`).join("");
  phrasalList.innerHTML = PHRASALS.map(x => `
    <li>
      <button class="speak btn ghost" aria-label="Odtwórz wymowę phrasala" data-text="${x.p}">🔊</button>
      <span class="en">${x.p}</span> — <span class="pl">${x.pl}</span>
      <span class="ex">(${x.ex})</span>
    </li>`).join("");

  document.querySelectorAll("button.speak[data-text]").forEach(btn => {
...
function pickCard() {
  ...
  if (due.length === 0) {
    current = null;
    fcFront.innerHTML = `<span class="wordText">🎉 Wszystko powtórzone!</span>`;
...
function renderCard(card, flipped) {
  ...
  fcFront.innerHTML = `<span class="wordText">${card.word}</span> <button class="speak btn ghost" id="fcSpeak" aria-label="Odtwórz wymowę">🔊</button>`;
  fcBack.innerHTML = `
    <div class="pl">${card.pl}</div>
    <div class="en">${card.def}</div>
    <div class="ex">${card.ex}</div>
  `;
  const speakBtn = document.getElementById("fcSpeak");
...
function schedule(id, quality) {
  ...
  const days = { 1:0, 2:1, 3:3, 4:7, 5:21 }[item.box] ?? 0;
  item.next = Date.now() + days * 24 * 60 * 60 * 1000;
  saveSRS(srs);
}

document.getElementById("againBtn").addEventListener("click", () => { if (!current) return; schedule(current.id, "again"); pickCard(); });
document.getElementById("goodBtn").addEventListener("click",  () => { if (!current) return; schedule(current.id, "good");  pickCard(); });
document.getElementById("easyBtn").addEventListener("click",  () => { if (!current) return; schedule(current.id, "easy");  pickCard(); });
document.getElementById("skipBtn").addEventListener("click",  () => { pickCard(); });
...
function makeQuestion() {
  ...
  questionText.textContent = `Tłumaczenie słowa: “${w.word}” to…?`;
  quizSpeak.onclick = () => speak(w.word);
